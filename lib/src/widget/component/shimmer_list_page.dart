...
class ShimmerListPage extends StatefulWidget {
  ...
  const ShimmerListPage({super.key, this.qtyPerLine = 3, this.shape = ShimmerShapeType.square, this.quantity = 3});
...
class _ShimmerListPageState extends State<ShimmerListPage> {
  ...
  double getSizeByLine() {
    ...
    double spacing = 3.0;
    return heightPerLine * widget.qtyPerLine + (spacing * widget.qtyPerLine - 1);
  }

  Widget getShape() {
    if (widget.shape == ShimmerShapeType.square) return ShimmerEffectWidget.rectangle(width: getSizeByLine(), height: getSizeByLine());
